import { type Request, type Response } from 'express';
import { createReportePubliSchema, evaluarReportePubliSchema } from '../validations/reportePubli.validation.js';
import { sendSuccess, sendError } from '../handlers/responseHandlers.js';
import { ReportePubliService } from '../services/reportePubli.service.js';

const reportePubliService = new ReportePubliService();

export async function obtenerReportesPubli(req: Request, res: Response): Promise<void> {
  try {
    const reportes = await reportePubliService.listarPendientes();
    sendSuccess(res, reportes, 'Reportes de publicaciones obtenidos', 200);
  } catch (error: any) {
    sendError(res, 'Error al obtener los reportes de publicaciones', 500);
  }
}

export async function crearReportePubli(req: Request, res: Response): Promise<void> {
  try {
    const { error } = createReportePubliSchema.validate(req.body, { abortEarly: false });
    if (error) {
      const mensajeError = error?.details?.map((d: any) => d?.message).join(', ') || 'Datos inválidos';
      sendError(res, mensajeError, 400);
      return;
    }

    const id_usuario = req.user?.id;
    if (!id_usuario) {
      sendError(res, 'Usuario no autenticado', 401);
      return;
    }

    const userAgent = req.headers['user-agent'] || 'Desconocido';
    const ipReporte = (req.headers['x-forwarded-for'] as string) || req.socket.remoteAddress || '0.0.0.0';

    const reporteCreado = await reportePubliService.crear(id_usuario, {
      id_publicacion: req.body.id_publicacion,
      motivo: req.body.motivo,
      detalle: req.body.detalle,
      ip_reporte: ipReporte,
      user_agent: userAgent,
    });

    sendSuccess(res, reporteCreado, 'Reporte de publicación creado correctamente', 201);
    
  } catch (error: any) {
    const message = error?.message || 'Error interno del servidor';
    let statusCode = 500;

    if (message.includes('NOT_FOUND:')) statusCode = 404;
    else if (message.includes('BAD_REQUEST:')) statusCode = 400;

    const cleanMessage = message.replace(/NOT_FOUND:|BAD_REQUEST:/g, '').trim();
    sendError(res, cleanMessage, statusCode);
  }
}

export async function evaluarReportePubli(req: Request, res: Response): Promise<void> {
  try {
    const id_reporte = Number(req.params.id_reporte);
    if (isNaN(id_reporte)) {
      sendError(res, 'ID de reporte inválido', 400);
      return;
    }

    const { error } = evaluarReportePubliSchema.validate(req.body);
    if (error) {
      const mensajeError = error?.details?.[0]?.message || 'Estado inválido';
      sendError(res, mensajeError, 400);
      return;
    }

    const reporteEvaluado = await reportePubliService.evaluar(id_reporte, req.body.estado);
    sendSuccess(res, reporteEvaluado, 'Reporte evaluado correctamente', 200);

  } catch (error: any) {
    const message = error?.message || 'Error interno del servidor';
    let statusCode = 500;

    if (message.includes('NOT_FOUND:')) statusCode = 404;

    const cleanMessage = message.replace(/NOT_FOUND:/g, '').trim();
    sendError(res, cleanMessage, statusCode);
  }
}